...
const Save = (props) => {
	...
	const handleShare = (e) => {
		...
		if (listId) {
			...
			if (displayName && props.achievements.length > 0) {
				axios
					.post('https://www.daigleportfolio.me/samples/achievement/save', {
						name: displayName,
						achievements: props.achievements,
					})
					.then((res) => {
	...
	const deleteList = (e) => {
		...
		if (listId === '61db929371bb51061f686efc') {
			...
			axios
				.get('https://www.daigleportfolio.me/samples/achievement/delete/' + listId)
				.then((res) => {
					alert('deleted');
		...
		<>
			<div id="save-wrapper">
				...
				<form>
					...
					></input>
					<button id="save-button" onClick={handleShare}>
						Save Your List
					</button>
				</form>
				{listId ? (
					<p id="list-id">
						Your List ID: {listId}
						<button id="copy-icon" onClick={copyToClip}>
			...
			<div id="delete-wrapper">
				{listId ? (
					<>
						<button id="delete-button" onClick={toggle}>
							Delete List (NO UNDO)
						</button>
						<Modal
							isShowing={isShowing}
